...
## 4. MCP サーバー（スキルが要求する場合）

一部のスキルは外部サービスとの連携に MCP（Model Context Protocol）サーバーを使用する。
`/setup` が `requires-mcp` を持つスキルを自動検出し、未設定の MCP があれば追加を提案する。

### 手動で追加する場合

```bash
# リモート MCP サーバー（HTTP transport）
...
`--scope project` を指定すると `.mcp.json` に保存され、チームで共有できる。
...
- [ ] `claude mcp list` で必要な MCP サーバーが設定済みであること

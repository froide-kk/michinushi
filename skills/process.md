...
| **Agent** | 判断・委譲・複数 Skill の組み合わせ | `AGENT.md` | `.claude/agents/<name>/AGENT.md` |
...
`AGENT.md` は `.claude/agents/<name>/AGENT.md` として配置されると Claude Code がサブエージェントとして自動認識する。michinushi の配布物は curl で直接 `.claude/agents/` に展開されるため、`/setup` でのコピー処理は不要。`/setup update` で最新版を取得すると差分を反映する。
...
```
        ...
        │  修正後は reviewer に再レビュー依頼（MUST 0件まで）│
        └────────────────────┬────────────────────────────┘
                             │ 設計レビュー通過（MUST 指摘 0件）
        ┌────────────────────▼────────────────────────────┐
        │               実装フェーズ                        │

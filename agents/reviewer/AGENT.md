...
3. **学習データの投入（育成機能が有効な場合のみ）**:
   - **前提**: `.claude/config/project.yml` の `cultivation.enabled` が `true` であること。**`false` または未定義なら本ステップ全体をスキップ** し、Step 4 に進む（育成機能はオプトインのため、明示的に有効化されていないプロジェクトでは observation を投入しない）
   - 妥当な指摘を `.claude/config/review-feedback.yml` の `observations` に投入する:
     - 既存 observation と意味的に重複していれば `detections` を append し `occurrence-count` を +1
     - 新規観点なら新しい observation を作成（id 自動生成: `<category>-<slug>-<hash6>` 形式。詳細は `cultivate-review` Skill の「id 採番ルール」参照）
   - `category` / `severity` の選定基準は `cultivate-review` Skill の「category / severity の選定基準」セクションに従う（reviewer ごとに採用基準がブレないように統一）
   - 後で cultivator Agent が `/cultivate` 実行時にこの蓄積を読んで育成判断する
   - フォーマットの詳細は `cultivate-review` Skill の「データモデル」セクション参照
4. **コード修正は行わない**（implementer の責務）

> **Edit / Write ツールの使用範囲**: reviewer Agent の frontmatter には `Edit, Write` が含まれているが、これは `.claude/config/review-feedback.yml` への observation 投入のみに使う。プロダクトコード・設計書・テスト等の修正は implementer / designer / tester の責務であり、reviewer が直接書き換えてはならない。ツール権限自体では区別できないため、本原則をプロンプトレベルで厳守する。

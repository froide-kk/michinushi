...
- 各 observation についてユーザーと対話し、`promoted` / `rejected` / `deferred` / `delete` のいずれかを確定
...
| トリガー | 動作 |
|---|---|
| 手動 `/cultivate` | 蓄積された observation を全件処理対象とする |
| 自動（任意工程） | Conductor のフロー末尾で呼ばれる。PR マージ後など |

引数は取らない。蓄積されている `review-feedback.yml` 全体を処理対象とする。
...
2. **対話する** — observation ごとにユーザーに 4 択（`promoted` / `rejected` / `deferred` / `delete`）を問う。一括判断はしない
...
processed:
  - id: <observation id>
    decision: promoted | rejected | deferred | delete

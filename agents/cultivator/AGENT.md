...
> **前提**: 育成機能はオプトインのため、`.claude/config/project.yml` の `cultivation.enabled` が `true` のときのみ動作する。`false` または未定義の場合、cultivator は起動しても何も処理せず「育成機能は無効」とユーザーに案内して終了する。
...
- `.claude/config/project.yml` の `cultivation.enabled` — 育成機能の有効化フラグ（**`true` のときのみ起動**。デフォルト `false` のオプトイン）

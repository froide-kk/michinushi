...
> ⚠️ **破壊的変更（2026-05 リリース）** — 配布物を `agents/` と `skills/` の2ディレクトリに分割し、ユーザー側でも `.claude/agents/` と `.claude/skills/` に分けて配置するようになりました。`/setup update` で移行できます。詳細は [Migration Guide](#migration-guide-from-previous-versions) を参照してください。
>
> ⚠️ **破壊的変更（2026-04 リリース）** — Skill と Agent の再構成を行いました。9つの Skill（architect / coder / docgen / ui-designer / unit-tester / e2e-tester / security-tester / tech-reviewer / biz-reviewer）が **6つの Agent**（architect / designer / implementer / tester / reviewer / analyst）+ **複数の参照 Skill** に再編されています。
...
> **`.claude/agents/` と `.claude/skills/` の git 管理について**: チームで同じバージョンを共有したい場合はリポジトリにコミットすることを推奨。各開発者が個別に取得する運用なら `.gitignore` に追加してください。

> **旧版（2026-04 以前）からの移行**: 旧 `/setup` が `.claude/agents/<name>.md`（フラットファイル）に Agent をコピーしていたため、上のコマンドで新構造を導入すると `.claude/agents/<name>.md` と `.claude/agents/<name>/AGENT.md` が共存します。導入後に `/setup` を実行すると旧フラットファイルの削除を案内します。

導入後の更新は `/setup update` で行います（内部で curl 取得 → 差分集計 → 個別確認のフローが実行されます）。
...
`.claude/agents/` 配下にディレクトリ単位で配置され、Claude Code がサブエージェントとして自動認識する。
...
```
.claude/
  agents/                    # <-- Michinushi 配布物（Agents）
    architect/AGENT.md       # Agent 定義
    designer/AGENT.md
...
- **Agent** (`AGENT.md`): 判断と委譲を担うサブエージェント。`.claude/agents/` に配置すれば Claude Code が自動認識

それぞれ別ディレクトリで管理するため、Skill と Agent が同名で衝突することはありません。

### Skill/Agent vs Config の分離

| | Skills/Agents (`skills/`, `agents/`) | Config (`config/`) |
|--|---|---|
...
skills/
  your-skill/
    SKILL.md   # 参照 Skill 定義
    *.md       # Supporting docs (optional)
```

種別ごとにディレクトリを分けて配置する（`agents/` と `skills/` の両方に同じ機能を置かない）。
...
`.claude/agents/<your-agent>/AGENT.md` として配置すれば Claude Code が自動認識する。
...
`/setup update` は新版を一時ディレクトリに取得し、差分を集計します。具体的には:

- `.claude/agents/<name>/AGENT.md` を新規配置（または既存と差分があれば上書き確認）
- `.claude/skills/<name>/AGENT.md`（旧 Agent ディレクトリ）を「削除候補」として提示
- `.claude/agents/<name>.md`（旧フラットファイル）も削除候補として提示

画面の指示に従って削除を承認してください。手動で実行する場合は [Installation](#installation) のコマンドを再実行してください。

#### 2. 旧 setup 経由のフラットファイル

旧 `/setup` で `.claude/agents/<name>.md`（フラットなファイル）にコピーしていたものは、新構成では `.claude/agents/<name>/AGENT.md`（ディレクトリ + AGENT.md）に変わります。フラットファイルが残っていると Claude Code が同名 Agent を二重認識する可能性があるため、`/setup update` または `/setup` で削除を承認してください。

### 2026-04 リリース: 9 Skill → 6 Agent + 参照 Skill 群

旧構成（9つの Skill）から新構成（6つの Agent + 参照 Skill 群）への移行手順:
...
手動で実行する場合は [Installation](#installation) のコマンドを再実行してください。

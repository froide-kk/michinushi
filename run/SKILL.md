...
Conductor はこのプロセスに従って工程を進行する。まず `process.md` を読み込むこと。
...
- **`#33` や `33`** → Issue 番号。`gh issue view <番号> --repo <owner>/<repo>` で読み取る
...
### Step 4: process.md のフローに従って工程を実行

Architect の分析結果に基づき、`process.md` に定義された設計フェーズ・実装フェーズの工程を順に実行する。
